...
if eta_policy in ['test'] :
    np.savetxt(FLAGS.log_dir+"/Ising2D10_LRtest_eta%.3e_%.3e.txt"%(eta0,etaMax),training_table[:,:2])
else :
    np.savetxt(FLAGS.log_dir+"/Ising2D10_CLR_eta%.3e_%.3e.txt"%(eta0,etaMax),training_table)
...
if eta_policy not in ['test'] :
    ...
    np.savetxt(FLAGS.log_dir+"/Ising2D10_output_eta%.3e_%.3e.txt"%(eta0,etaMax),table)
