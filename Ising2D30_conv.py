...
flags.DEFINE_string("data_dir",
                    get_data_path(
                        dataset_name = "kelvinchngphysicist/2d30", #all mounted repo
...
eta_policy = 'test' # 'cyclical_traig' # 'test'
etaMax = 10
eta0 = 1e-6
...
W_conv3 = weight_variable([filter_d,filter_h,n_feature_map2,n_feature_map3])
b_conv3 = bias_variable([n_feature_map3])

h_conv3 = tf.nn.relu(conv2d(h_conv2, W_conv3) + b_conv3)

# Fourth Convolution Layer
W_conv4 = weight_variable([filter_d,filter_h,n_feature_map3,n_feature_map4])
...
W_fc1 = weight_variable([26*26*n_feature_map2, n_fully_connected_neuron]) 
b_fc1 = bias_variable([n_fully_connected_neuron])

h_conv1_flat = tf.reshape(h_conv2, [-1, 26*26*n_feature_map2])
...
if eta_policy in ['test'] :
    np.savetxt(FLAGS.log_dir+"/Ising2D30_LRtest_eta%.3e_%.3e.txt"%(eta0,etaMax),training_table[:,:2])
else :
    np.savetxt(FLAGS.log_dir+"/Ising2D30_CLR_eta%.3e_%.3e.txt"%(eta0,etaMax),training_table)
